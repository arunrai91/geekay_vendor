...
class BundlePack extends \Magento\Catalog\Block\Product\AbstractProduct implements IdentityInterface
{
    ...
    public function getPackJsonConfig(PackInterface $pack, $items)
    {
        $data = [
            ...
            'parent_info' => [
                'price' => $this->getPrice($this->getProduct()),
                'qty' => 1
            ],
    ...
    {
        $toExclude = $this->taxConfig->discountTax() ? [] : ['tax', 'weee'];
        return (float)$product->getPriceInfo()->getPrice('final_price')->getAmount()->getValue($toExclude);
    }
}
