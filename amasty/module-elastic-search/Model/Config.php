...
use Amasty\ElasticSearch\Model\Source\Tokenizer;
...
class Config
{
    ...
    public function getTokenizer(int $storeId): string
    {
        if ($this->useCustomAnalyzer($storeId)) {
            $tokenizer = Tokenizer::WHITESPACE;
        } else {
            $tokenizer = $this->getModuleConfig('connection/tokenizer', $storeId) ?? Tokenizer::WHITESPACE;
        }

        return (string)$tokenizer;
    }

    public function isSynonymsCaseSensitive(?int $storeId = null): bool
    {
        return (bool)$this->getModuleConfig('catalog/synonym_case_sensitive', $storeId);
    }

    public function isStopWordsCaseSensitive(?int $storeId = null): bool
    {
        return (bool)$this->getModuleConfig('catalog/stop_word_case_sensitive', $storeId);
    }
}
